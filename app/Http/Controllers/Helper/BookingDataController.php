...
//        .' '.$details->estimate_bill;
        $return = [
            ...
            'later_booking_time'=>!empty($details->later_booking_time) ? $details->later_booking_time : "",
...
//                }])
                ...
                $booking = BookingCheckout::select('id','merchant_id','payment_method_id','card_id','segment_id','vehicle_type_id','country_area_id','user_id','service_package_id','service_type_id','is_geofence','base_area_id','auto_upgradetion','number_of_rider','total_drop_location','price_card_id','pickup_latitude','pickup_longitude','pickup_location','drop_latitude','drop_longitude','drop_location','booking_type','estimate_bill','additional_information','additional_notes','waypoints','estimate_distance','estimate_time','map_image','promo_code','wheel_chair_enable','baby_seat_enable','ac_nonac','bill_details','payment_option_id')
                    ->find($booking_id);
            }
            $string_file = $this->getStringFile($booking->merchant_id);
            if (!empty($booking)){
