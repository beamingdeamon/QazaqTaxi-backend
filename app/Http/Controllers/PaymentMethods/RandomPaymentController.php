        ...
        if (empty($diff) || $diff > 3) {
            ...
            try {
                ...
                $response = $mpesa->STKPush([
                    ...
                    'PhoneNumber' => $phone, // Phone number should look like 254700000000
                    'AccountReference' => $phone, // you can pass phone number
                    'CallBackURL' => $url . 'api/mpessapayment_confirmation?user_id=' . $user_id . '&type=' . $type . '&amnt=' . $amount . '&merchant_id='.$user->merchant_id, // the callback get repsonse from mpesa server. Check implementation in the next fucntion
                ]);
            } catch (Exception $e) {
...
//        $appkey = '7e870232a16b4f89bf602cd13c2ede86';
    ...
    public function OrangeMoneyPush(Request $request){
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://testom.orange.bf:9008/payment',
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        try{
            ...
            $data = array(
                ...
                "Source"=> [
                    ...
                    "CardExpiration"=> $card->exp_year.$card->exp_month,
            ...
            if ($response->Approved == true) {
                ...
                if($response->IsoResponseCode==12){
                    ...
                    throw new \Exception(trans("$string_file.some_thing_went_wrong"));
            }else{
    ...
    public function WaveFail(Request $request){
        dd($request->all());
    }
}
