...
//            ];
            ...
            if($order->order_status == 1)
            {
                if($order->ServiceType->type == 6)
                {
                    ...
                    array_push($action_buttons,$accept);
                }
                else
                {
...
//                }
            ...
            elseif($order->order_status ==10 && $order->ServiceType->type == 6)
            {
                $action_buttons = [
                    [
                        'button_text'=> trans("$string_file.deliver_order"),
                        'button_text_colour'=>"FFFFFF",
                        'button_action'=>"DELIVER",
                    ],
                ];
            }
...
//                });
    ...
    public function acceptOrder(Request $request)
    {
        try {
            $business_seg = $request->user('business-segment-api');
            $request->request->add(['latitude'=>$business_seg->latitude,'longitude'=>$business_seg->longitude]);
            $message =  $this->acceptOrderByBusinessSegment($request,$business_seg);
        }
        catch (\Exception $e) {
            $message = $e->getMessage();
            return $this->failedResponse($message);
    ...
    // deliver order to user // self pickup order
...
//                $ref->checkReferral($arr_params);
        ...
        // order settlement
        $this->orderSettlement($order);
        $message = trans("$string_file.user_collected_order");
        return $this->successResponse($message, []);
    }
