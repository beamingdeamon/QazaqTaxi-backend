...
//        p($segment->Segment->slag);
        ...
        // Get Merchant Brands
        $brands = App\Models\Brand::with(['Segment' => function($query) use($segment){
            $query->where('id',$segment->segment_id);
        }])->whereHas("Segment",function($query) use($segment){
            $query->where('id',$segment->segment_id);
        })->where('merchant_id',$merchant_id)->where('delete','=',NULL)->get();
        $arr_brands = [];
        foreach($brands as $brand){
            $arr_brands[$brand->id] = $brand->Name($merchant_id);
        }
...
//            'product_cover_image'=>'required_without:id|mimes:jpg,jpeg,png',
            'product_cover_image' => 'required_if:id,!=,null|sometimes|mimes:jpeg,png,jpg,gif,svg|dimensions:min_width=' . $width . ',min_height=' . $height,
...
//            'title' => 'required|max:255|unique:product_variants,product_title,' . $id . ',id,product_id,' . $product_id,
        ...
        try {
            ...
            if($product->status == 2 && $request->status == 1){
                return array('error' => trans("$string_file.product_variant_status_error"));
            }
            $is_title_show = 0;
