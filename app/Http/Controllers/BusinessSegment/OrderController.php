...
//    }
  ...
  // Pending orders delivery
    public function pendingOrderDelivery(Request $request)
    {
        //$business_segment = get_business_segment();
        $order = new Order;
        $request->request->add(['status'=>"PENDING_ORDER_DELIVERY"]);
        $arr_orders = $order->getOrders($request,true);
        $business_seg = get_business_segment(false);
        $hide_user_info_from_store = $business_seg->Merchant->ApplicationConfiguration->hide_user_info_from_store;
        $req_param['merchant_id'] = $business_seg->merchant_id;
        $arr_status = $this->getOrderStatus($req_param);
        $search_route =  route('business-segment.pending-order-delivery');
        $request->request->add(['search_route'=>$search_route]);
        $search_view = $this->orderSearchView($request);
        $time_format = $business_seg->Merchant->Configuration->time_format;
        $arr_search = $request->all();
        return view('business-segment.order.pending-order-delivery',compact('arr_orders','arr_status','search_view','arr_search','hide_user_info_from_store','time_format'));
    }
...
//p($order);
    ...
    public function deliverOrder(Request $request,$id)
    {
        ...
        $data = $request->all();
        //  p('inn');
        return view('business-segment.order.deliver-order',compact('order','arr_status','data','business_segment','hide_user_info_from_store'));
    }
...
//            return redirect()->back()->withErrors($message);
    ...
    public function orderPickupVerify(Request $request){
        try {
            $return = $this->orderOTPVerification($request);
        ...
        if($order->ServiceType->type == 6)
        {
            return redirect()->route('business-segment.deliver-order',$order->id)->withSuccess($message);
    ...
    public function acceptOrder(Request $request,$id)
    {
        ...
        try {
            ...
            $request->request->add(['latitude'=>$business_segment->latitude,'longitude'=>$business_segment->longitude]);
            $message =  $this->acceptOrderByBusinessSegment($request,$business_segment);
        }
        catch (\Exception $e) {
            $message = $e->getMessage();
            return redirect()->back()->withErrors($message);
        ...
        return redirect()->route('business-segment.pending-process-order')->withSuccess($message);
    }
    ...
    public function deliverOrderRequest(Request $request,$id)
    {
        ...
        try {
            ...
            $order= Order::find($id);
            if(!empty($order->id) && $order->order_status == 10)
            {

                $order->order_status = 11; // delivered order
                ...
                $request->request->add(['order_status'=>11,'latitude'=>$business_seg->latitude,'longitude'=>$business_seg->longitude]);
...
//                $ref->checkReferral($arr_params);
        ...
        // order settlement
        $this->orderSettlement($order);
        $order_rejected_message = trans("$string_file.user_collected_order");
        return redirect()->route('business-segment.completed-order')->withSuccess($order_rejected_message);
    }
}
