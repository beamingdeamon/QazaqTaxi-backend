...
class ExpireDocumentController extends Controller
{
    ...
    function getUserDocumentGoingToExpire($currentDate, $reminder_last_date, $merchant_id, $user_id = NULL)
    {
        $dateBetween = array($currentDate, $reminder_last_date);
        $userVehicleDocumentWith = ['UserVehicleDocument' => function ($o) use ($dateBetween) {
            $o->whereHas('Document', function($q) use($where)
            {
                $q->where('expire_date',1);
            });
        ...
        $raw_query = User::select('id','first_name','last_name','email','phoneNumber','country_id')->where([['merchant_id', '=', $merchant_id], ['driver_delete', '=', NULL]])
            ->with(['UserDocument' => function ($query) use ($dateBetween) {
                ...
                'UserVehicles' => function ($d_v) use ($dateBetween, $userVehicleDocumentWith) {
                $d_v->with($userVehicleDocumentWith)
                    ->whereHas('UserVehicleDocument', function ($p) use ( $dateBetween) {
            ...
            ->where(function ($q) use ($dateBetween, $userVehicleDocumentWith) {
                $q->whereHas('UserDocument', function ($q) use ( $dateBetween) {
                    ...
                    ->orWhereHas('UserVehicles', function ($r) use ($dateBetween, $userVehicleDocumentWith) {
                    $r->with($userVehicleDocumentWith)
                        ->whereHas('UserVehicleDocument', function ($s) use ( $dateBetween) {
