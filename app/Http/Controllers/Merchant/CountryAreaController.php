...
class CountryAreaController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       // $self_pickup = $merchant_segment_food_grocery > 0 ? true : false;
        ...
        return view('merchant.area.index', compact('areas', 'config', 'segment_group_vehicle', 'segment_group_handyman', 'area_id', 'category_vehicle_type_module','arr_status','info_setting','arr_areas','self_pickup'));
    }
...
//            $arr_segment = $request->input('segment');
    ...
    public function addStep2(Request $request, $area_id)
    {
        ...
        $is_demo = false;
        $segment_group_id = [1, 3]; // With Carpooling
        $arr_segment_services = $this->getMerchantSegmentServices($merchant_id, '', $segment_group_id,[],NULL,false,[],NULL,"DELIVERY");
...
//        $segment_exist = $this->checkAreaVehicleExist($area_id, $vehicle_type_id);
        $arr_segment_services = $this->getMerchantSegmentServices($merchant_id, '', $segment_group_id, [], NULL, true,[],NULL,"DELIVERY");
...
//        }
        ...
        /************* for group 2 segments end **************/
        $info_setting = InfoSetting::where('slug','COUNTRY_AREA_CONFIGURATION')->first();
        return view('merchant.area.form-step5', compact('merchant', 'config', 'area',
            'arr_segment_services', 'arr_selected_segment_service','info_setting','is_demo'));
    }
