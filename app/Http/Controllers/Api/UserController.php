...
//        $newArray = $transaction->toArray();
        ...
        foreach ($transaction as $value) {
             ...
             else
            {
             ...
             if(!empty($value->wallet_transfer_id) && !empty($value->WalletTransfer) && $value->narration == 12)
             {
             ...
             elseif(!empty($value->wallet_transfer_id) && !empty($value->WalletTransfer) && $value->narration == 13)
             {
...
//                })],
        ...
        } elseif ($request->type == 2) // forgot password
        {
            ...
            $validator = Validator::make($request->all(), $fields,['user_name.exists' => $validationMsg]);
            if ($validator->fails()) {
                $errors = $validator->messages()->all();
                return $this->failedResponse($errors[0]);
            }
        ...
        return $this->successResponse($message, array('auto_fill' => $auto_fill, 'otp' => (string)$otp, 'default_otp_enable' => $default_otp_enable, 'default_otp' => $default_otp));
    }
...
//        return response()->json(['result' => "1", 'message' => trans('api.message103'), 'data' => $driver]);
    ...
    public function FavouriteDrivers(Request $request)
    {
        ...
        $drivers = FavouriteDriver::with('Driver')->where([['user_id', '=', $user_id]])->get();
        if (empty($drivers->toArray())) {
