...
//    }
    ...
    public function getAllExpireUserDocument($merchant_id = NULL,$document_verification_status = 2,$pagination = true)
    {
        ...
        $query = User::select('id','first_name','merchant_driver_id','last_name','country_id','email','phoneNumber','merchant_id')->where([['user_delete', '=', NULL]])
            ->with([
            ...
            'UserDocument' => function ($p) use ($where) {
                    $p->where($where);
                    $p->where('status',1);
                    $p->whereHas('Document', function($q) use($where)
                    {
            ...
            ->where(function ($q) use ($where, $userVehicleDocumentWith) {
                ...
                ->orWhereHas('UserDocument', function ($s) use ($where, $userVehicleDocumentWith) {
                    $r->with($userVehicleDocumentWith)
                    ->whereHas('UserVehicleDocument', function ($s) use ($where) {
                    $s->where($where);
                    $s->where('status',1);
